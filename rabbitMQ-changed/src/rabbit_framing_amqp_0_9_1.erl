...
-type(amqp_method_field_name() ::
       ( delivery_tag | routing_key | mandatory | queue
       | message_count | exchange | if_unused | nowait
       | arguments | multiple | routing_key | queue
       | no_ack | if_unused | ticket | destination
       | source | routing_key | exclusive | nowait
       | arguments | exchange | ticket | ticket
       | destination | ticket | source | nowait
       | routing_key | nowait | arguments | routing_key
       | requeue | delivery_tag | requeue | redelivered
       | exchange | routing_key | prefetch_size | delivery_tag
       | ticket | queue | passive | durable
       | exclusive | auto_delete | nowait | arguments
       | ticket | version_minor | consumer_tag | no_local
       | queue | message_count | consumer_count | no_ack
       | arguments | reply_code | reply_text | version_major
       | server_properties | ticket | queue | mechanisms
       | locales | nowait | exchange | exchange
       | client_properties | mechanism | response | locale
       | challenge | message_count | if_empty | queue
       | routing_key | nowait | channel_max | frame_max
       | nowait | consumer_tag | heartbeat | prefetch_count
       | exchange | channel_max | frame_max | heartbeat
       | global | virtual_host | arguments | capabilities
       | insist | immediate | known_hosts | consumer_tag
       | delivery_tag | reply_code | cluster_id | reply_text
       | class_id | method_id | message_count | multiple
       | requeue | out_of_band | consumer_tag | channel_id
       | nowait | active | redelivered | consumer_tag
       | active | reply_code | reply_text | class_id
       | delivery_tag | method_id | realm | ticket
       | exclusive | routing_key | ticket | passive
       | ticket | active | write | read
       | ticket | queue | queue | ticket
       | ticket | exchange | type | requeue
       | passive | exchange | response | durable
