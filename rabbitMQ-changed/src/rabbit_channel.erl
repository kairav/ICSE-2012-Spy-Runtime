...
handle_method(#'queue.declare'{queue       = QueueNameBin,
    ...
    case rabbit_amqqueue:with(
        ...
        {ok, MessageCount, ConsumerCount} ->
        	%% CHANGE Edited so that an exception is thrown
            %% return_queue_declare_ok(QueueName, NoWait, MessageCount, ConsumerCount, State);
            rabbit_misc:protocol_error(precondition_failed, "~s in use", [rabbit_misc:rs(QueueName)]);
        {error, not_found} ->
