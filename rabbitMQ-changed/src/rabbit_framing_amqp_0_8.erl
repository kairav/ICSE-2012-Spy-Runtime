...
-type(amqp_method_field_name() ::
       ( consumer_tag | redelivered | passive | durable
       | result | exclusive | auto_delete | routing_key
       | consumer_tag | nowait | arguments | consumer_tag
       | requeue | response | queue | message_count
       | mandatory | consumer_count | identifier | delivery_tag
       | routing_key | ticket | queue | exchange
       | exchange | routing_key | nowait | arguments
       | delivery_tag | identifier | durable | nowait
       | multiple | reply_code | ticket | requeue
       | multiple | queue | ticket | operation
       | auto_delete | nowait | global | content_checksum
       | message_count | exchange | reply_code | delivery_tag
       | redelivered | exchange | queue | ticket
       | heartbeat | no_ack | queue | integer_3
       | reply_code | if_unused | no_local | if_empty
       | nowait | prefetch_count | delivery_tag | immediate
       | queue | string_1 | global | redelivered
       | message_count | exchange | delivery_tag | ticket
       | queue | mandatory | reply_text | exchange
       | channel_max | routing_key | arguments | no_ack
       | exclusive | nowait | ticket | string_2
       | realm | result | integer_4 | consumer_tag
       | reply_text | prefetch_size | routing_key | ticket
       | prefetch_count | server_properties | exclusive | global
       | queue | prefetch_size | consumer_tag | prefetch_count
       | reply_code | immediate | reply_text | requeue
       | delivery_tag | class_id | passive | method_id
       | delivery_tag | consumer_tag | exchange | no_local
       | exchange | exchange | no_ack | routing_key
       | out_of_band | active | nowait | virtual_host
       | no_local | active | consumer_tag | active
       | consumer_tag | exclusive | capabilities | reply_code
       | reply_text | reply_text | details | consumer_tag
       | integer_op | reply_code | nowait | integer_1
       | reply_text | read | class_id | insist
       | method_id | exchange | dtx_identifier | routing_key
       | consumer_tag | version_major | version_minor | ticket
       | requeue | consumer_tag | exclusive | routing_key
       | ticket | meta_data | mechanisms | staged_size
       | locales | queue | operation | write
       | client_properties | string_result | mechanism | ticket
       | response | routing_key | table | queue
       | locale | exchange | routing_key | nowait
       | challenge | ticket | exchange | type
       | consumer_tag | consumer_tag | passive | prefetch_size
       | delivery_tag | channel_max | ticket | frame_max
       | mandatory | internal | consumer_tag | nowait
       | arguments | consumer_tag | frame_max | host
       | heartbeat | cluster_id | integer_2 | ticket
       | consume_rate | exchange | content_size | if_unused
       | nowait | nowait | immediate | known_hosts
       | message_count | ticket | integer_result | string_op
       | known_hosts | exchange | routing_key | ticket
       | queue | identifier )).
