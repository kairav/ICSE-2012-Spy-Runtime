...
-type(amqp_method_field_name() ::
       ( no_ack | arguments | message_count | exchange
       | if_unused | nowait | queue | mandatory
       | if_unused | exclusive | ticket | destination
       | source | routing_key | nowait | ticket
       | nowait | arguments | multiple | ticket
       | ticket | redelivered | destination | reply_code
       | source | routing_key | requeue | nowait
       | arguments | multiple | version_major | prefetch_size
       | ticket | queue | passive | redelivered
       | ticket | durable | exclusive | auto_delete
       | nowait | arguments | consumer_tag | reply_text
       | server_properties | no_local | queue | immediate
       | exchange | message_count | consumer_count | arguments
       | routing_key | exchange | ticket | queue
       | exchange | routing_key | consumer_tag | nowait
       | requeue | arguments | if_empty | consumer_tag
       | nowait | ticket | queue | prefetch_count
       | nowait | nowait | consumer_tag | routing_key
       | channel_max | frame_max | heartbeat | ticket
       | client_properties | global | channel_max | frame_max
       | ticket | heartbeat | routing_key | virtual_host
       | cluster_id | capabilities | mechanism | insist
       | delivery_tag | known_hosts | exchange | reply_code
       | message_count | response | reply_text | class_id
       | method_id | queue | no_ack | out_of_band
       | locale | delivery_tag | channel_id | exchange
       | active | message_count | consumer_tag | active
       | delivery_tag | nowait | reply_code | delivery_tag
       | reply_text | class_id | method_id | ticket
       | routing_key | requeue | challenge | realm
       | exclusive | requeue | passive | queue
       | exchange | active | queue | write
       | read | version_minor | ticket | mechanisms
       | locales | response | ticket | exchange
       | type | passive | durable | routing_key
       | auto_delete | delivery_tag | internal | nowait
       | arguments )).
