...
-type(amqp_method_field_name() ::
       ( type | integer_result | durable | exclusive
       | auto_delete | nowait | arguments | consumer_tag
       | requeue | delivery_tag | queue | routing_key
       | message_count | mandatory | consumer_count | identifier
       | consumer_tag | nowait | ticket | integer_1
       | ticket | multiple | durable | queue
       | global | exchange | reply_code | routing_key
       | multiple | nowait | no_local | arguments
       | immediate | delivery_tag | identifier | ticket
       | ticket | auto_delete | queue | reply_code
       | ticket | integer_3 | queue | nowait
       | queue | internal | exchange | exclusive
       | message_count | exchange | string_result | delivery_tag
       | redelivered | cluster_id | ticket | no_ack
       | routing_key | delivery_tag | queue | queue
       | mandatory | if_unused | if_empty | nowait
       | prefetch_count | ticket | string_1 | global
       | redelivered | channel_max | message_count | ticket
       | operation | reply_code | delivery_tag | ticket
       | queue | immediate | exchange | version_minor
       | ticket | routing_key | string_2 | arguments
       | no_ack | exclusive | routing_key | nowait
       | reply_text | exclusive | operation | server_properties
       | consumer_tag | reply_text | prefetch_size | exchange
       | prefetch_count | global | mechanisms | staged_size
       | requeue | prefetch_size | delivery_tag | consumer_tag
       | prefetch_count | integer_4 | reply_code | immediate
       | reply_text | class_id | virtual_host | method_id
       | active | consumer_tag | result | no_local
       | exchange | no_ack | out_of_band | nowait
       | reply_text | no_local | capabilities | active
       | consumer_tag | exchange | active | routing_key
       | consumer_tag | reply_code | requeue | insist
       | reply_text | read | details | exchange
       | consumer_tag | result | content_size | reply_code
       | consumer_tag | reply_text | exchange | class_id
       | dtx_identifier | routing_key | identifier | method_id
       | consumer_tag | content_checksum | exchange | requeue
       | routing_key | consumer_tag | queue | integer_op
       | version_major | nowait | realm | ticket
       | exclusive | exchange | table | passive
       | ticket | queue | locales | write
       | delivery_tag | client_properties | mechanism | response
       | prefetch_size | locale | consumer_tag | consumer_tag
       | challenge | exchange | meta_data | response
       | routing_key | passive | nowait | routing_key
       | host | channel_max | delivery_tag | mandatory
       | frame_max | exchange | heartbeat | nowait
       | arguments | frame_max | ticket | heartbeat
       | known_hosts | ticket | nowait | exchange
       | consumer_tag | if_unused | nowait | consumer_tag
       | message_count | string_op | known_hosts | redelivered
       | integer_2 | ticket | consume_rate | queue
       | routing_key | passive )).
